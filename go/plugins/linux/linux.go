...
func Discovery(jsonInput map[string]interface{}, errContext *[]map[string]interface{}) {
	...
	for _, credential := range jsonInput[constants.Credentials].([]interface{}) {
		...
		if isValid {
			jsonInput[constants.Result] = map[string]interface{}{constants.Ip: jsonInput[constants.Ip].(string)}

			jsonInput[constants.CredentialProfileID] = credential.(map[string]interface{})[constants.CredentialId].(float64)

			return
		}
	}

	jsonInput[constants.CredentialProfileID] = constants.InvalidCredentialCode
}
